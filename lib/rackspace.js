...
let deleteRequest = (url, callback) => {
  return unirestFactory('DELETE', url, callback);
};
...
let getAllMailbox = (offset, callback) => {
	// [GET] https://api.emailsrvr.com/v1/customers/(customer account number)/domains/(domain name)/rs/mailboxes/(mailbox name)
	//TODO handle paging
	var url = 'https://api.emailsrvr.com/v1/customers/' + AccountNumber + '/domains/' + Domain + '/rs/mailboxes?size=250&offset='+offset
		+'&fields=name,lastLogin,enabled,size,currentUsage,createdDate';
	request(url, (response) => {
		if(response.body.total > offset+250){
			getAllMailbox(offset+250, callback);
		}
		callback(response.body, offset);
	});
};
...
let deleteMailBox = (email, callback) => {

  //[DELETE] https://api.emailsrvr.com/v1/customers/(customer account number)/domains/(domain name)/rs/mailboxes/(mailbox name)
  //var email = mailbox.Username + '@' + Domain;

  deleteRequest('https://api.emailsrvr.com/v1/customers/' + AccountNumber + '/domains/' + Domain + '/rs/mailboxes/' + email, (response) => {
    if(response.statusType !== 2) {
    	console.error(chalk.bold.red('Error deleting %s mailbox : %s'), email,
      	response.body.badRequestFault ? response.body.badRequestFault.message : response.body);
		}
		else {
			console.info(chalk.green('\nMailbox %s deleted.'), email);
		}
